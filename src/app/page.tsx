...
const Button = ({ 
  ...
  className={cn(
    ...
    // Common green styles for both 'default' and 'success'
...
const App = () => {
  ...
  const [carBreakdowns, setCarBreakdowns] = useState({});
  ...
  const DriverDashboard = ({ onLogout, carBreakdowns }) => {
    ...
    // Simulated driver info (replace with actual data in a real application)
    const driverInfo = {
      id: 1,
      name: "John Doe",
        ...
        <Card className="w-full max-w-2xl transform transition-all duration-300 hover:scale-105">
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <Button type="submit" className="w-full text-xl py-4">Save and Upload</Button>
            </form>
          </CardContent>
        </Card>
      </div>
  ...
  const CompanyList = ({ onBack, onSelectCompany }) => {
                  ...
                  {filteredCompanies.map((company) => (
                    <tr key={company.id} className="hover:bg-gray-50 transition-colors duration-200">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-lg font-medium">
                        <Button variant="link" onClick={() => onSelectCompany(company.id)} size="lg">
                          View Details
                        </Button>
                      </td>
                    </tr>
  ...
  const CompanyDetails = ({ companyId, onBack, onCarBreakdown }) => {
    ...
    const handleBreakdownToggle = (carId) => {
      ...
      onCarBreakdown(carId, !company.cars.find(car => car.id === carId).isBrokenDown);
    };
  ...
  const DriverManagement = ({ onBack, onViewProfile }) => {
    const [searchTerm, setSearchTerm] = useState('');
    ...
    const confirmRemoveDriver = (password) => {
      // In a real application, you would verify the password here
      if (password === 'admin') { // Example password
        setDrivers(drivers.filter(driver => driver.id !== driverToRemove));
                    ...
                    <tr key={driver.id} className="hover:bg-gray-50 transition-colors duration-200">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-lg font-medium">
                        <div className="flex space-x-2">
                          <Button variant="link" onClick={() => onViewProfile(driver.id)} size="sm">
                            View Profile
                          </Button>
                          <Button 
                            variant={driver.status === 'Active' ? 'destructive' : 'success'} 
                            size="sm"
  ...
  const DriverProfile = ({ driverId, onBack }) => {
